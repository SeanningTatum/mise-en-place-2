...
description: Updates the living high-level architecture document with route maps, feature flows, and changelog. Use proactively after implementing features, adding new routes, or making architectural changes. Keeps the architecture overview current for planning and documentation.
---
...
Use the Task tool with `subagent_type: "architecture-tracker"`:
...
**After `context-keeper`**: Context-keeper updates detailed technical docs; architecture-tracker updates the visual overview.

Execution order:
```
implement-feature → context-keeper → architecture-tracker
```

## What Makes This Different from context-keeper

| Aspect | context-keeper | architecture-tracker |
|--------|----------------|---------------------|
