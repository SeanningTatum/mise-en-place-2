...
const YOUTUBE_VIDEO_EXTRACTION_PROMPT = `You are a recipe extraction expert. Watch this cooking video and extract a complete structured recipe.

CRITICAL INSTRUCTIONS FOR TIMESTAMPS:
1. For EACH cooking step, provide the EXACT timestamp (in seconds) when that step begins in the video
2. Watch the video carefully and note when each cooking action starts
3. Include the duration of each step (seconds until the next step begins)
4. Timestamps must be accurate to help users jump to specific parts of the video
...
5. Each step should have an accurate timestamp for when it's demonstrated in the video
...
/**
 * Model for YouTube video processing (more accurate for video understanding)
 */
export const VIDEO_GEMINI_MODEL = "gemini-2.5-pro";
 ...
 * This method passes the YouTube URL directly to Gemini Pro for accurate timestamp extraction
 */
export async function extractRecipeFromYouTube(
  ...
  log.info({ youtubeUrl, model: VIDEO_GEMINI_MODEL }, "Starting YouTube video recipe extraction with Gemini Pro");
    ...
    log.info(
      {
        ...
        stepsWithTimestamps: extracted.steps.filter(s => s.timestampSeconds !== null).length,
      },
